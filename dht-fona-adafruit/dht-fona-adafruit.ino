...
Adafruit_MQTT_Publish humid = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/humidity2");
...
void setup() {
  ...
  Serial.println(F("@iLabAfrica,...Initializing"));
...
//uint32_t temp= temperature;
  ...
  Serial.print(F("\nSending temprature and humidity values "));

  Serial.print("...");
  if (! temp.publish(temperature)) {
  //if (! photocell.publish(cPayload)) {
...
if (! humid.publish(humidity)) {
  ...
  //}
  //Apparently this delay is important, I have no idea why.
  delay(10000);
